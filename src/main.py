...
google_maps = f"https://maps.googleapis.com/maps/api/js?key={api_key}&callback=window.initMap"

app = dash.Dash(__name__)

app.layout = html.Div([
    dcc.Store(id="map-data"),
    html.Div(id="map", style={"height": "500px", "width": "100%"}),
    html.Button("Get Info", id="get-info-button"),
    html.Label(id="info-label"),
    html.Img(id="result"),
])
...
def download_google_maps_image(lat, lon, zoom, dimensions, api_key):
    ...
    if response.status_code == 200:
        ...
        image = cv2.imdecode(np.frombuffer(image_buffer.read(), np.uint8), cv2.IMREAD_COLOR)
        return image
    else:
        print(f"Error: Unable to download image. Status code: {response.status_code}")
        return None

def extract_water_edge(image: np.ndarray, water_color: np.ndarray = np.array([249, 192, 156]), tolerance: int = 20) -> np.ndarray:
...
def generate_map(map_data):
    ...
    img = download_google_maps_image(map_data["lat"], map_data["lon"], map_data["zoom"], (640, 640), api_key)
